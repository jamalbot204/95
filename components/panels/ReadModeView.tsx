      ...
      >
        <div className={`bg-[var(--aurora-msg-ai-bg)] text-[var(--aurora-text-on-surface)] border border-[var(--aurora-border)] p-6 sm:p-8 rounded-lg markdown-content shadow-2xl ${getFontSizeClass(readModeFontSizeLevel)} transition-all duration-200`}>
             <ReactMarkdown 
                ...
                components={{ 
                    code: CodeBlock, 
                    p: ({ children, ...props }: any) => <div {...props}>{processNodeContent(children)}</div>,
                    li: ({ children, ...props }: any) => <li {...props}>{processNodeContent(children)}</li>,
                    blockquote: ({ children, ...props }: any) => <blockquote {...props}>{processNodeContent(children)}</blockquote>,
                    div: ({ children, ...props }: any) => <div {...props}>{processNodeContent(children)}</div>,
